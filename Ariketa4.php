...
<head>
<title>4.Ariketa</title> <!-- web orrian agertuko den izenburua -->
...
<body>
<div class="container">
        <form action="Ariketa4.php" method="GET"> <!-- produktuak bilatzeko formularioa -->
            <a href=""><i class="fa fa-plus" aria-hidden="true" id="plus"></i></a> <!-- produktuak gehitzeko ikonoa -->
        ...
        if ($result->num_rows > 0) { //emaitz guztiak inprimatzeaz zihurtatzen da
            while ($row = $result->fetch_assoc()) { //emaitzen lerroak inprimatzeaz zihurtatzen den buklea
                if (str_contains(strtolower($row["Izena"]), strtolower($bilatu)) && str_contains(strtolower($row["Mota"]), strtolower($mota))) { //$bilatu parametroak izena eta $mota parametroak motarekin bat egiten badute, lerro hori autatuko da eta if barnekoa exekutatuko da sartutako izena eta mota dituen lerroarekin
                    echo "<tr>";
